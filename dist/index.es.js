...
const useMapRotation = () => {
  const map = useMap();
  const [rotationDegree, setRotationDegree] = useState(map.getView().getRotation() / (Math.PI / 180));
  const setRotate = useCallback(degree => {
  ...
  return jsx(ControlGroup, {
    children: jsx(Button, Object.assign({
      ...
      children: jsx(InnerButton, {
        children: isFull ? jsx(FullScreenExitIcon, {
          size: 16
        }) : jsx(FullScreenIcon, {
          size: 16
        })
...
const ZoomFeature = () => {
  var _a;
  const map = useMap();
  const [zoomAmount, setZoomAmount] = useState((_a = map.getView().getZoom()) !== null && _a !== void 0 ? _a : 0);
...
const CustomMarker = ({
  ...
  const map = useMap();
  const annotationRef = useRef(new Feature$2(new Point$3(fromLonLat(center))));
  const annotationLayerRef = useRef(new VectorLayer({
    source: new VectorSource({
      features: [annotationRef.current]
    })
...
function CustomMultiPoint({
  ...
  useEffect(() => {
    ...
    const features = geometry.getPoints().map((point, index) => {
      ...
      const pointFeature = new Feature$2(point);
...
const TextMarker = ({
  ...
  const map = useMap();
  const annotationRef = useRef(new Feature$2({
    geometry: new Point$3(fromLonLat(center))
  }));
  const annotationLayerRef = useRef(null);
  useEffect(() => {
    if (annotationLayerRef.current && zIndex) {
...
const ImageMarker = ({
  ...
  const map = useMap();
  const imageRef = useRef(new ImageLayer({
...
const Map = /*#__PURE__*/forwardRef(({
  ...
  return jsxs(MapContext.Provider, Object.assign({
